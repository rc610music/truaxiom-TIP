...
| `GET /health` `summary` organization, product, and project lines | Same as Registry v1 | `summarySources[0..2]` is `postgres` after a successful reconcile. The other summary lines stay seed. |
| `GET /health` `registry.counts` | Same as Registry v1 | This is the count block to compare with Neon. |
| `GET /v1/snapshot` and `GET /v1/collections/{organizations,products,projects}` | Registry rows overlaid on the bootstrap snapshot | Other snapshot collections stay seed. |
| `GET /v1/context/organization` | Mixed | Organization, products, and projects come from the overlay. Agents, modules, knowledge, tasks, and recommendations stay seed. |
| `modules`, `agents`, `knowledgeObjects`, `tasks`, `recommendations`, `ingestionSources`, `contentMaps`, `graphNodes`, `graphEdges`, `activity` | In-memory seed | Neon has empty `modules`, `agents`, `knowledge_objects`, `tasks`, `graph_nodes`, `graph_edges`, and `activity_events` tables. The API does not read them. `recommendations`, `ingestion_sources`, and `content_maps` tables are not in Neon. |
